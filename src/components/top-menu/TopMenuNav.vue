<template>
  ...
  <div class="menu" v-if="menus" v-bind:style="{ marginLeft: '-' + margin.left + 'px', marginRight: '-' + margin.right + 'px' }">
    <template v-for="menu in menus" v-bind:key="menu.path">
      <top-menu-nav-list ref="topMenuNavList" v-bind:menu="menu" v-bind:status="menu.status" v-on:collapse-other="handleCollapseOther(menu)"></top-menu-nav-list>
    </template>

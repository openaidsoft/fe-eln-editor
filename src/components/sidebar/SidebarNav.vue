<template>
  ...
  <div class="menu" v-if="menus">
    ...
    <div v-for="menu in menus" v-bind:key="menu.path">
      <sidebar-nav-list
        ref="sidebarNavList"
        v-bind:menu="menu"
...
export default {
  ...
  created() {
    this.menus.forEach((each, i) => {
      if (each.menu) {
        this.childrenMenuh = each;
      }
      if (each.has_menu_yn == "Y") {
        this.hasChildrenMenu = each;
      }
    });
  },
  data() {

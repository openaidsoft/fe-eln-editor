  ...
  <div>
    ...
    <div id="header" class="app-header" v-bind:class="{ 'app-header-inverse': appOptions.appHeaderInverse }">
      ...
      <div class="navbar-header">
        ...
        <router-link to="/" class="navbar-brand"><span class="navbar-logo"></span> <b>Cleve</b>Lab </router-link>
...
import DropDown from "../cmm/DropDown.vue";
...
export default {
  ...
  created() {
    let _langObj = undefined;
    const _lang = this.$cookies.get("lang");
    if (_lang) {
      if (_lang === "kr") {
        _langObj = this.arrayOfObjects.filter((r) => r.value === "kr")[0];
      } else if (_lang === "en") {
        _langObj = this.arrayOfObjects.filter((r) => r.value === "en")[0];
      }
      this.localeChange(_lang);
    }
    const currLang = _langObj === undefined ? this.locale_lang : this.$cookies.get("lang");
    this.object = this.arrayOfObjects.filter((r) => r.value === currLang)[0];
  },
  methods: {
    methodToRunOnSelect(payload) {
      this.$cookies.set("lang", payload.value);
      this.localeChange(payload.value);
      this.object = payload;
      location.reload(); // The reason why page reloading is to update for Ag-Grid...
    },
    toggleSidebarMobile() {
